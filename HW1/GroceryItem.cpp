...
_price = std::exchange(rhs._price, 0.0);
...
// operator>>(...)
  ...
  stream >> std::quoted(upcCode) >> delimiter
         >> std::quoted(brandName) >> delimiter
         >> std::quoted(productName) >> delimiter
         >> price;

  if (stream) // Check for successful input before modifying the object
  {
